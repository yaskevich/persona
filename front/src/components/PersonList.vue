<template>
  <div v-if="isLoaded">
    ...
    <el-row v-for="(value, key) in filtered()" :gutter="20" :key="key">
      ...
      <el-col :span="4">
        <div class="grid-content bg-purple"><i :class="value.sex === 1 ? 'el-icon-male' : 'el-icon-female'"></i>
          {{ value.firstname }}</div>
      </el-col>
...
const form = reactive({ firstname: '', lastname: '', patroname: '', sex: 1, wikidata: '' } as IPerson);
...
const confirm = () => {
  formRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      // console.log(result);
      if (data && 'id' in data) {
        persons.unshift({ ...form } as IPerson);
