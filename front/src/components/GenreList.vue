  ...
  <el-row v-for="(value, key) in genres" :gutter="20" :key="key">
    <el-space>
      <el-popconfirm :title="loc('confirmdel')" :confirmButtonText="loc('yes')" :cancelButtonText="loc('no')"
        ...
        <template #reference>
          <el-button type="danger" link size="small" icon="el-icon-delete" plain class="full-width"></el-button>
        </template>
      </el-popconfirm>
      <el-button type="primary" plain @click="showDialog(value.title, value.id)">{{ value.title }}</el-button>
    </el-space>
  </el-row>
  ...
  <el-dialog :title="loc('genre')" v-model="dialogVisible" width="30%">
    <el-form :model="form" ref="formRef" label-width="100px" :rules="rules" :inline="true" @submit.prevent.native>
      <el-form-item prop="title">
        <el-input :placeholder="loc('title')" v-model="form.title" class="text-input" ref="formInputRef" :autofocus="true"
